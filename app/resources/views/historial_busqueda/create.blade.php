<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Nuevo Historial</h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('historial_busqueda.store') }}">
                    @csrf
                    <div class="mb-4">
                        <label class="block text-gray-700 mb-1">Resultado Búsqueda</label>
                        <input type="text" name="resultado_busqueda" value="{{ old('resultado_busqueda') }}" class="w-full border border-gray-300 rounded p-2">
                    </div>
                    <div class="mb-4">
                        <label class="block text-gray-700 mb-1">Fecha</label>
                        <input type="date" name="fecha_busqueda" value="{{ old('fecha_busqueda') }}" class="w-full border border-gray-300 rounded p-2">
                    </div>
                    <div class="mb-4">
